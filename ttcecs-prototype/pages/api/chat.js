 ...
 * Combines sentiment analysis + intelligent response generation
 * NOTE: This file replaces the separate /api/sentiment.js for cleaner architecture
 */
export default async function handler(req, res) {
  ...
  try {
    ...
    if (HF_TOKEN) {
      ...
      console.warn('⚠️  HF_TOKEN not found in .env.local. Using rule-based sentiment.');
    }

    // 2. Generate contextual response
    const reply = generateResponse(message, sentiment);

    // 3. Simulate realistic typing delay
 ...
 * Based on message content + detected sentiment
 */
function generateResponse(message, sentiment) {
  ...
  // Intent detection with detailed responses
  ...
  if (lowerMsg.includes('branch') || lowerMsg.includes('location') || lowerMsg.includes('address') || lowerMsg.includes('office')) {
    return "Our head office is at No:1735, 18th Main Rd, Anna Nagar West, Chennai, Tamil Nadu 600040. We also have branches in Hyderabad. Check the Contact section for full details and maps!";
  }
  ...
  if (lowerMsg.includes('time') || lowerMsg.includes('hours') || lowerMsg.includes('schedule') || lowerMsg.includes('when')) {
  ...
  if (lowerMsg.includes('thank') || lowerMsg.includes('thanks')) {
    return "You're welcome! I'm always here if you need more help. Have a great day! 😊";
  }

  // Positive sentiment - enthusiastic, engaging tone
